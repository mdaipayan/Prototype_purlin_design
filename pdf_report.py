...
def _check_row(chk: CheckResult, styles):
    ...
    return [
        Paragraph(chk.label, styles["body"]),
        Paragraph(f"{chk.value} {chk.unit}", styles["body"]),
        Paragraph(f"≤ {chk.limit} {chk.unit}", styles["body"]),
        Paragraph(chk.status, status_style),
    ]
...
def generate_pdf_report(
    ...
    inp_data = [
        ...
        ["Yield strength of steel", "Fy", inp.fy, "N/mm²"],
        ["Modulus of elasticity", "E", inp.E, "N/mm²"],
    ...
    load_data = [
        ...
        ["Kx (along-slope factor)", "X / √(X²+Y²)", round(res.Kx, 6), "—"],
        ["Ky (cross-slope factor)", "Y / √(X²+Y²)", round(res.Ky, 6), "—"],
        ["Combo I — DL+LL+CL (↓)", "(DL+LL+CL)·Kx·Ps", round(res.w_combo1, 3), "kg/m"],
    ...
    story += _section_head("4. Z-Section Properties", styles)
    dims_data = [
    ...
    props_data = [
        ...
        ["Section modulus bottom", "Z1xx-bot", f"{res.section.Z1xx_bot:.2f}", "mm³"],
        ["Section modulus right",  "Zyy-right", f"{res.section.Zyy_right:.2f}", "mm³"],
        ["Cross-sectional area",   "A", round(res.section.area, 3), "cm²"],
    ...
    story += _section_head("6. Lateral Buckling — Permissible Bending Stress (IS 801 cl. 6.3b)", styles)
    lb_data = [
        ["Parameter", "Value", "Unit"],
        ["Unbraced length (L_u)", round(res.L_unbraced, 3), "m"],
        ["Iyc = Iyy/2", round(res.Iyc, 3), "cm⁴"],
        ["Sxc = Zxx-top", round(res.Sxc, 3), "cm³"],
        ["λ = L²·Sxc / (d·Iyc)", round(res.lambda_val, 2), "—"],
        ["Fb (computed)", round(res.Fb, 2), "N/mm²"],
        ["F_basic = 0.6·Fy", round(res.F_basic, 2), "N/mm²"],
        ["Fb (adopted, min of above)", round(res.Fb, 2), "N/mm²"],
    ]
    story.append(_table(lb_data, [W*0.52, W*0.28, W*0.20]))

    # ── STEP 10: Stress Checks ─────────────────
    story += _section_head("7. Bending Stress Checks", styles)
    sc_data = [["Load case / location", "fb_actual (N/mm²)", "Limit (N/mm²)", "Status"]]
    ...
    # ── STEP 11: Deflection ────────────────────
    story += _section_head("8. Deflection Check", styles)
    ...
    # ── STEP 12: Overlap ───────────────────────
    story += _section_head("9. Purlin Overlap Check", styles)
    lap_data = [
        ["Parameter", "Value", "Unit"],
    ...
    # ── SUMMARY ────────────────────────────────
    story += _section_head("10. Design Summary", styles)
    ...
    provide_text = (
        f"<b>PROVIDE PURLIN Z-{int(sec.D)} × {sec.t} mm</b> with "
